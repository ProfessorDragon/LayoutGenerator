...
Result<std::shared_ptr<SettingV3>> ObjectSettings::parse(std::string const& key,
...
SettingNodeV3* ObjectSettings::createNode(float width) {
    return ObjectSettingsNode::create(std::static_pointer_cast<ObjectSettings>(shared_from_this()),
                                      width);
}

std::vector<std::vector<int>> ObjectSettingsNode::objectIdLayout = {};

std::unordered_set<int> ObjectSettingsNode::objectIdWhitelistable = {};

void ObjectSettingsNode::generateLayout(std::unordered_map<std::string, uint32_t> customObjects) {
    objectIdLayout = {
        // gamemode, speed, size portals
        {12, 13, 47, 111, 660, 745, 1331, 1933, 10, 11, 2926, 99, 101},
    ...
    if (customObjects.contains("profdragon.bigportal/big-portal")) {
        objectIdLayout[0].push_back(customObjects["profdragon.bigportal/big-portal"]);
    }

    // generate whitelist
    for (auto& row : objectIdLayout) {
        for (auto objectId : row) {
            objectIdWhitelistable.insert(objectId);
        }
    }
}

bool ObjectSettingsNode::isWhitelistable(int objectId) {
    return objectIdWhitelistable.contains(objectId);
}

ObjectSettingsNode* ObjectSettingsNode::create(std::shared_ptr<ObjectSettings> setting,
                                               float width) {
...
bool ObjectSettingsNode::init(std::shared_ptr<ObjectSettings> setting, float width) {
    ...
    getNameMenu()->setLayout(
        RowLayout::create()->setAxisAlignment(AxisAlignment::Start)->setAutoScale(false));
    getNameMenu()->setLayoutOptions(
        AnchorLayoutOptions::create()->setAnchor(Anchor::TopLeft)->setOffset({10.f, -15.f}));

    m_toggleMenu = CCMenu::create();
    m_toggleMenu->setContentSize({width, height});
    m_toggleMenu->setLayout(ColumnLayout::create()->setAxisReverse(true));

    for (auto& row : objectIdLayout) {
        auto menu = CCMenu::create();
        menu->setContentWidth(width);
        menu->setLayout(
            RowLayout::create()->setAutoScale(false)->setGap(row.size() >= 12 ? 2.f : 6.f));

        for (int j = 0; j < row.size(); j++) {
            auto objectId = row[j];
            auto frame = ObjectToolbox::sharedState()->intKeyToFrame(objectId);
            if (std::string_view(frame).empty()) continue;
            ...
            if (offSprite->getContentHeight() < 20.f) offSprite->setContentHeight(20.f);

            auto onSprite = GameObject::createWithSpriteFrameName(frame);
            onSprite->setScale(onSprite->getContentHeight() > 45.f ? .5f : .75f);
            if (onSprite->getContentHeight() < 20.f) onSprite->setContentHeight(20.f);

            auto toggle = CCMenuItemToggler::create(offSprite, onSprite, this,
                                                    menu_selector(ObjectSettingsNode::onToggle));
            toggle->m_notClickable = true;
            toggle->setTag(objectId);
